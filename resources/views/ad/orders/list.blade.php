  ...
  <div class="col-sm-12">
    <form action="" method="get">
      ...
      <div class="col-sm-2">
        ...
        <a href="/hi/orders/list-all">Reset</a>
      </div>
    </div>
    </form>
  </div>
      ...
      <table class="table table-hover table-bordered">
        ...
        <tr>
          ...
          <?php
            switch ($order->admin_status) {
              ...
              case "seen":
                  ...
                  break;
              case "sent":
                  echo '<span class="label label-table label-info">Đã xác nhận là đã xem order</span>';
                  break;
          ...
          <td>
            ...
            <a href="/hi/orders/detail/{{$order->id}}">
              <button type="button" class="btn btn-xs btn-primary btn-rounded waves-effect waves-light">Detail</button>
            </a>
          </td>
        </tr>
        @endforeach
...
<script>
  ...
  function destroyTeacher(order_id, name) {
    let deleteConfirm = confirm("Do you want delete " + name + "?")
    if(deleteConfirm) {
      $.ajax({
        url : "{{ url('/hi/orders/destroy') }}",
        type : "post",
        dataType:"text",
        data : {
          id: order_id
